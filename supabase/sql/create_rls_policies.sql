-- Row Level Security (RLS) policies for payment_references table
-- This ensures users can only access payment references they're authorized to see
...
-- Allow organization members to view payment references for their organization (alternative policy)
...
-- Debug function to check RLS status
CREATE OR REPLACE FUNCTION public.check_payment_references_rls()
RETURNS TABLE(
  policy_name text,
...
BEGIN
  ...
  SELECT 
    pol.policyname,
    CASE 
      WHEN pol.polpermissive = 'f' THEN 'Restrictive'
      ELSE 'Permissive'
    END as policy_status
  FROM pg_policies pol
  WHERE pol.tablename = 'payment_references';
END;
$$;