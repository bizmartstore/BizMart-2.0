...
serve(async (req) => {
  ...
  try {
    ...
    const { action, userId, title, message, link, icon } = await req.json()
    ...
    if (action === 'send_push') {
      ...
      if (!serviceAccountRaw) {
        console.error("[firebase-admin-wrapper] Missing FIREBASE_SERVICE_ACCOUNT secret")
      ...
      // Get user FCM tokens
      const { data: fcmTokens, error: fcmTokenError } = await supabase
        .from('fcm_tokens')
        .select('token')
        .eq('user_id', userId)

      if (fcmTokenError || !fcmTokens || fcmTokens.length === 0) {
        return new Response(JSON.stringify({ success: true, message: 'No tokens' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        })
      }
