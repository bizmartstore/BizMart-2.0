...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "products" }, () => {
        console.log("[Marketplace] Product change detected, refetching...");
        refetchProducts();
      })
      .subscribe();
  ...
  const startChat = async (sellerId: string) => {
    ...
    if (!existing) {
      await (supabase as any).from("conversations").insert({ participant_1: user.id, participant_2: sellerId });
    }
    navigate("/messages");
  };
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-3 mt-4">
        ...
        <div className="relative mb-3">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products..." className="pl-9 pr-9 text-sm h-9 rounded-xl" />
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="flex-1 flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
            {categories.map((cat) => (
              <button key={cat} onClick={() => setSelectedCategory(cat)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] font-bold transition-all ${selectedCategory === cat ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground"}`}>
                {cat === "all" ? "All" : cat.charAt(0).toUpperCase() + cat.slice(1)}
              </button>
            ))}
          </div>
          <button onClick={() => setShowSort(!showSort)} className={`flex-shrink-0 p-2 rounded-xl transition-all ${showSort ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
        ...
        {showSort && (
          <div className="mb-3 bg-card border border-border rounded-xl p-2 shadow-sm">
            ...
            <div className="flex flex-wrap gap-1.5">
              {sortOptions.map((opt) => (
                <button key={opt.value} onClick={() => { setSortBy(opt.value); setShowSort(false); }} className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all ${sortBy === opt.value ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2" onClick={() => navigate(`/store/${sellerId}`)} role="button">
                  <div className="w-7 h-7 rounded-lg bg-accent overflow-hidden flex items-center justify-center">
                    {seller?.store_image ? <img src={seller.store_image} className="w-full h-full object-cover" alt="" /> : <Store className="h-3.5 w-3.5 text-primary" />}
                  </div>
                  <div>
                ...
                <button onClick={() => startChat(sellerId)} className="flex items-center gap-1 text-[10px] font-bold text-primary bg-primary/10 px-2 py-1 rounded-full">
