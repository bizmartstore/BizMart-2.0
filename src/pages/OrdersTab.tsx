...
import { Search, CheckCircle2, XCircle, Loader2, RefreshCw, Printer, Package } from "lucide-react";
...
export default function OrdersTab() {
  ...
  const updateStatus = async (orderId: string, newStatus: string) => {
    ...
    try {
      const isPrint = selectedOrder?.file_name;
    ...
    <div className="space-y-3">
            ...
            <div key={o.id} className="bg-card rounded-xl border border-border p-3 flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  {o.file_name ? <Printer className="h-4 w-4 text-purple-500" /> : <Package className="h-4 w-4 text-primary" />}
                  <span className="font-bold text-xs text-foreground truncate">
                    {o.file_name ? `Print: ${o.file_name}` : `Order #${o.id.slice(0, 8)}`}
                  </span>
                </div>
      ...
      {selectedOrder && (
        <div className="bg-card rounded-xl border border-border p-4">
          {/* Selected order UI would go here */}
          <p>Selected order details</p>
        </div>
      )}
