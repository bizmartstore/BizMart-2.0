...
export default function ClubPage() {
  ...
  const handleSubmitApplication = async () => {
    if (!appForm.full_name.trim() || !appForm.reason.trim() || !appForm.business_type.trim() || !appForm.products_to_sell.trim()) {
  ...
  const handleActivateSeller = async () => {
    ...
    try {
      ...
      const { data: sp, error: spError } = await (supabase as any).from("seller_profiles").insert({
        user_id: user.id,
        store_name: `${profile?.first_name || ""}'s Store`,
      }).select().single();
        ...
        {!membership && !checkingMembership && (
          ...
          <div className="bg-card rounded-2xl p-4 border border-border space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSubmitApplication} disabled={submittingApp} className="w-full">
