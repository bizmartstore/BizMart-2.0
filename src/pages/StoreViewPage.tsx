...
import { ArrowLeft, MapPin, Star, Package, MessageCircle } from "lucide-react";
  ...
  useEffect(() => {
    const loadStore = async () => {
      if (isOfficial) {
        ...
        // For now, show empty – future: filter by seller_id
        setProducts([]);
        setLoading(false);
      }
    };
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] font-bold transition-all ${
                    selectedCategory === cat                      ? "bg-primary text-primary-foreground shadow-sm"
