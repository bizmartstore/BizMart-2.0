...
export async function triggerNotification(params: NotifyParams) {
  const { title, message, type, userId, role, link, icon = "🔔" } = params;
  ...
  try {
    const { data, error: pushError } = await supabase.functions.invoke("send-notification", {
      body: { title, message, targetUserId: userId, targetRole: role, link, icon },
    });
...
export const notifyAdminNewOrder = (customerName: string, amount: number) =>
  triggerNotification({
