...
export default function SellersTab() {
  ...
  const filteredSellers = sellers.filter(s =>     !search || (s.store_name || "").toLowerCase().includes(search.toLowerCase())
  );
            ...
            {filteredSellers.map(s => (
              <div key={s.id} className="bg-card rounded-xl border border-border p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${s.is_active ? 'bg-[hsl(var(--success))]/20 text-[hsl(var(--success))]' : 'bg-muted text-muted-foreground'}`}>{s.is_active ? "Active" : "Inactive"</span>
                </div>
          ...
          <div className="space-y-2 max-h-[500px] overflow-y-auto">
            {filteredApps.map(a => (
              <div key={a.id} className="bg-card rounded-xl border border-border p-3">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="font-bold text-xs">{a.profiles?.first_name} {a.profiles?.last_name}</p>
                    <p className="text-[10px] text-muted-foreground">{a.business_type}</p>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${a.status === 'approved' ? 'bg-[hsl(var(--success))]/20 text-[hsl(var(--success))]' : a.status === 'pending' ? 'bg-warning/20 text-warning' : 'bg-destructive/20 text-destructive'}`}>{a.status}</span>
                  <button onClick={() => setSelectedApp(a)} className="p-1.5 rounded-lg bg-muted hover:bg-muted/80"><Eye className="h-3.5 w-3.5" /></button>
                </div>
                <p className="text-[10px] text-muted-foreground mb-2 line-clamp-2">{a.reason}</p>
                {a.admin_notes && <p className="text-[10px] text-muted-foreground mt-2">Admin note: {a.admin_notes}</p>}
              </div>
            ))}
