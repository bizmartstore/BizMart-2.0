      ...
      <div className="flex items-center justify-between">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div>
            ...
            <Input type="number" value={form.original_price} onChange={e => setForm(f => ({ ...f, original_price: e.target.value }))} className="text-xs h-8" />
          </div>
          <div>
            <Label className="text-[10px]">Stock *</Label>
            <Input type="number" value={form.stock} onChange={e => setForm(f => ({ ...f, stock: Number(e.target.value) }))} className="text-xs h-8" />
          </div>
        </div>
        <div>
          <Label className="text-[10px]">Category</Label>
          <Input value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} placeholder="e.g. notebooks" className="text-xs h-8" />
        </div>
        <div>
          ...
          {form.image ? (
            <div className="relative w-full h-28 rounded-lg overflow-hidden border border-border mt-1">
              ...
              <button onClick={() => setForm(f => ({ ...f, image: "" }))} className="absolute top-1.5 right-1.5 bg-destructive text-destructive-foreground rounded-full p-0.5">
                <X className="h-3 w-3" />
              </button>
            </div>
          ) : (
