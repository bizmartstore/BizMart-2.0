  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        ...
        const { error } = await (supabase as any).from("products").insert({ ...payload, id: `seller-${Date.now()}` });
        if (error) throw error;
        ...
        <div className="bg-card rounded-xl p-3 border border-border space-y-2">
          ...
          <div>
            ...
            {form.image ? (
              ...
              <button onClick={() => fileRef.current?.click()} disabled={uploading} className="w-full h-24 border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center gap-1 mt-1 hover:bg-muted/50 transition-colors">
